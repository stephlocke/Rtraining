...
getsessions <- function(url) {
    html(url) %>% 
      ...
      rbind.fill %>% 
      data.table %>%
      .[,day:=as.Date(str_sub(url,-8,-1),format="%Y%m%d")]
}
...
write.csv(sessionswithoutsubmissions, "sessionswithoutsubmissions.csv")
save(finaldataset,sessionswithoutsubmissions,file = "webscrapingresults.RData")
