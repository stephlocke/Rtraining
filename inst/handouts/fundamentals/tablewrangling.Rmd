...
Task | Generic syntax | Example(s)\*
------------- | ------------- | -------------
INNER JOIN | Y[X, nomatch=0L] | `lookupDT[irisDT,nomatch=0L]`
...
NULL count in every col by group | DT[ , lapply(.SD, function(x) sum(is.na(x))), by] | `irisDT[ , lapply(.SD,function(x) sum(is.na(x))), by=Species]`
Cardinality in every column by group | DT[ , lapply(.SD, uniqueN), by] | `irisDT[ , lapply(.SD,uniqueN), by=Species]`
